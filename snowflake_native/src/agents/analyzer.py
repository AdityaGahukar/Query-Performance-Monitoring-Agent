...
class QueryPerformanceAnalyzer:
    ...
    def analyze_finding(
        ...
        formatted_user_prompt = {
            "detected_issues": json.dumps(formatted_issues, indent=2, default=str),
            "query_history": json.dumps(snapshot.query_history, indent=2, default=str),
            "operator_stats": json.dumps(pruned_ops, indent=2, default=str),
            "warehouse_load": json.dumps(snapshot.warehouse_load, indent=2, default=str),
            "query_attribution": json.dumps(snapshot.query_attribution, indent=2, default=str),
        }
        ...
        timeout_seconds = self.settings.llm.request_timeout_seconds
        ...
        while True:
            try:
                ...
                class SimplifiedRecommendationSchema(BaseModel):
                ...
                class AnalysisOutputSchema(BaseModel):
                    ...
                    recommendations: list[SimplifiedRecommendationSchema]
                ...
                # Check if this is a rate limit or quota exceeded error to back off longer
                err_msg = str(e).lower()
                if "quota" in err_msg or "429" in err_msg or "rate limit" in err_msg:
                    sleep_time = 15.0 + 5.0 * retries_count
                    logger.info(f"Rate limit detected. Sleeping for {sleep_time}s to reset quota...")
                else:
                    sleep_time = 1.0 * retries_count
            ...
            for rec in recs_data:
                ...
                recommendations.append(
                    Recommendation(
                        ...
                        evidence=", ".join(rec.get("evidence")) if isinstance(rec.get("evidence"), list) else (rec.get("evidence") or ""),
                    )
                )
