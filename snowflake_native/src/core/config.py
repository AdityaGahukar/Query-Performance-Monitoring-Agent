...
class LLMSettings(BaseSettings):
    """
    Generic LLM provider settings (supporting Gemini and Nvidia).
    """

    model_config = SettingsConfigDict(
        env_prefix="LLM_",
        extra="ignore",
        protected_namespaces=("settings_",),  # suppress conflict warning for model field
        env_file=_env_file,
        env_file_encoding="utf-8",
    )
    ...
    provider: str = Field(
        default="nvidia",
        description="LLM provider name: 'nvidia'.",
    )
    model: Optional[str] = Field(
        default=None,
        description="LLM model identifier to use (e.g. 'meta/llama-3.1-8b-instruct').",
    )
    ...
    prompt_version: str = Field(
        default="v1_default",
        description="Active version of the system and user prompt templates.",
    )
    request_timeout_seconds: int = Field(
        default=60,
        description="Maximum seconds to wait for an LLM response.",
        ge=10,
        le=300,
    )
    max_retries: int = Field(
...
class Settings(BaseSettings):
    ...
    def resolve_and_validate_llm(self) -> "Settings":
        provider_name = self.llm.provider.lower()
        if provider_name not in {"gemini", "nvidia"}:
            raise ValueError(f"LLM_PROVIDER must be 'gemini' or 'nvidia'. Got '{self.llm.provider}'")

        if provider_name == "gemini":
            api_key = self.llm.api_key or self.gemini.api_key
            if not api_key:
                raise ValueError("API key for Gemini must be provided via LLM_API_KEY or GEMINI_API_KEY.")
            self.llm.api_key = api_key
            if not self.llm.model:
                self.llm.model = self.gemini.model_name or "gemini-3.5-flash"
        elif provider_name == "nvidia":
            if not self.llm.api_key:
                raise ValueError("API key for NVIDIA must be provided via LLM_API_KEY.")
            if not self.llm.model:
                self.llm.model = "meta/llama-3.1-8b-instruct"
        return self
